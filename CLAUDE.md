...
- **Structure rules**: `src/structure_checker/rules.example.json` — defines heading patterns and numbering rules. `scanner.py` 只识别 markdown 标题（ATX `#` 可无空格/全角空格/BOM 容忍、setext `===`、无 `#` 的"第X章 标题"行首锚定 + 编号后空白），跳过代码围栏，不再全文正则扫描（正文"在第二章中"/"12.9亿"不再误报）。`N.` 小节规则含 `(?=\D|$)` 前瞻防小数/日期误判。建树以 markdown 标题层级
（`#` 数量）优先：「## 1. 概述」按作者意图作为 H2 二级标题，不再因规则判定为
"目"(level3) 误报 hierarchy_gap；真跳级（`# 第一章` 直接 `### 1.`）仍报。
setext `===` 标题文本行以 heading_level=1 参与建树。已知既有缺口：罗马占位
「第X章」→10、全角数字/句点字符类仅 ASCII、合并标题「第1章 第1节」只取首编号。
