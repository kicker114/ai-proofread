...
- **Structure rules**: `src/structure_checker/rules.example.json` — defines heading patterns and numbering rules. `scanner.py` 只识别 markdown 标题（ATX `#` 可无空格/全角空格/BOM 容忍、setext `===`、无 `#` 的"第X章 标题"行首锚定 + 编号后空白），跳过代码围栏，不再全文正则扫描（正文"在第二章中"/"12.9亿"不再误报）。全角归一化（全角数字/句点/拉丁、繁體節、Unicode 罗马 Ⅻ→XII、小写 iv）命中且偏移映射回原文；装饰前缀支持全角数字（第１部分）。`N.` 规则含 `(?=\D|$)` + 量词检查防小数（12.9亿 / 12. 5亿）。建树以 markdown 标题层级（`#` 数量）优先：「## 1. 概述」作为 H2 编号节不误报 hierarchy_gap；真跳级（`# 第一章`→`### 1.`）仍报。合并标题「第1章 第1节」取全编号（紧凑/普通行/`第1.` 前缀），叙述性提及被排除，同一行按 line_id 同层嵌套，跨行同号仅当前后都带节行且节号连续才合法。罗马严格判定：非法序列（IIX）→ number_missing、`第X章` 占位在混合体系判 number_missing、`第I章`=1 等合法罗马不误伤、体系切换（罗马→阿拉伯）不比较连续性；节级罗马（第IV节）已支持。测试 `tests/test_structure_scanner.py`（41 例）。
