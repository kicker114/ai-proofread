...
- **QA 发现（已修，commit 待提交）**：`phase0_structure` 原对全文正则扫描
  （`第X章`/`N.`），正文含"在第二章中""12.9亿"会误报。`scanner.py` 重写为识别
  markdown 标题（ATX `#` 可无空格/全角空格/BOM 容忍、setext `===`、无 `#` 的
  "第X章 标题"普通行行首锚定 + 编号后空白），并跳过代码围栏；`N.` 规则加
